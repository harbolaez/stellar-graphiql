...
class ScalarInput extends React.PureComponent<ScalarInputProps, {}> {
  ...
  componentDidMount() {
    ...
    if (
      ...
      input.focus();
      const value = isObject ? input.state.value : input.value;
      if (!isObject) {
      ...
      <span style={{ color }}>
        ...
        <Input
          style={{
            ...
            outline: "none",
            padding: 0,
            width: `${Math.max(3, value.length)}ch`
          }}
          ref={ref => {
            this._ref = ref;
          }}
          ...
          <span style={{ color: styleConfig.colors.def }}>
            <Input
              style={{
                ...
                width: `${Math.max(10, operationDisplayName.length)}ch`
              }}
              autoComplete="false"
...
class Explorer extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    const addOperation = (kind: NewOperationType) => {
      ...
      console.log("=====newOperationDef", newOperationDef);
    ...
    const actionsEl = !!actionsOptions.length && (
      <div
        style={{
          minHeight: "50px",
          maxHeight: "50px",
      ...
      >
        ...
        >
          <Select
            onChange={value => this._setAddOperationType(value)}
